...
export async function getCoachInterventionQueue(coachId: string): Promise<InterventionQueue> {
  ...
  // Generate recommendations for all athletes
  const allRecommendations: InterventionRecommendation[] = [];

  for (const athleteId of athleteIds) {
    try {
      const recommendations = await generateInterventionRecommendations(athleteId);
      allRecommendations.push(...recommendations);
    } catch (error) {
      console.error(`Failed to generate recommendations for athlete ${athleteId}:`, error);
    }
  }
