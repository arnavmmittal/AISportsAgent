...
-- Helper function to get current user's schoolId and role
CREATE OR REPLACE FUNCTION auth.user_school_id()
RETURNS TEXT AS $$
  SELECT "schoolId" FROM "User" WHERE id = auth.uid()
...
CREATE POLICY "Users can view users in same school"
  ON "User" FOR SELECT
  USING ("schoolId" = auth.user_school_id());
...
CREATE POLICY "Users can view own audit logs"
  ...
  USING ("userId" = auth.uid() OR auth.user_role() = 'ADMIN');
...
CREATE POLICY "Users can view knowledge base for their school"
  ...
  USING ("schoolId" IS NULL OR "schoolId" = auth.user_school_id());
...
CREATE POLICY "Coaches can manage team config"
  ...
  USING (
    auth.user_role() IN ('COACH', 'ADMIN')
    AND "schoolId" = auth.user_school_id()
  )
  WITH CHECK (
    auth.user_role() IN ('COACH', 'ADMIN')
    AND "schoolId" = auth.user_school_id()
...
DO $$
BEGIN
  RAISE NOTICE 'RLS policies successfully enabled on all tables!';
  RAISE NOTICE 'Security features:';
  RAISE NOTICE '  - Multi-tenant isolation by schoolId';
  RAISE NOTICE '  - Role-based access control (ATHLETE, COACH, ADMIN)';
  RAISE NOTICE '  - Coach-athlete consent enforcement';
  RAISE NOTICE '  - Privacy-first chat summary access';
  RAISE NOTICE '  - Crisis alert escalation paths';
END $$;
